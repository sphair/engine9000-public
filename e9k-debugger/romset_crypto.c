/*
 * COPYRIGHT © 2026 Enable Software Pty Ltd - All Rights Reserved
 *
 * https://github.com/alpine9000/engine9000-public
 *
 * See COPYING for license details
 */

/*
 * Neo Geo CMC50 and PCM2 ROM decryption helpers.
 *
 * This file contains C99 adaptations of MAME's Neo Geo protection helpers:
 *   src/devices/bus/neogeo/prot_cmc.cpp
 *   src/devices/bus/neogeo/prot_pcm2.cpp
 *   src/devices/bus/neogeo/prot_sma.cpp
 *
 * Original MAME source license: BSD-3-Clause
 * Original MAME copyright-holders: S. Smith, David Haywood, Fabio Priuli,
 * Razoola, Mr.K
 */

#include <stdint.h>
#include <stdlib.h>
#include <string.h>

#include "romset_crypto.h"

#define ROMSET_NGH264_CMC50_GFX_KEY 0x3f
#define ROMSET_NGH256_CMC42_GFX_KEY 0xad
#define ROMSET_NGH253_CMC42_GFX_KEY 0x06

static const uint8_t romset_crypto_cmc42Type0T03[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Type0T12[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Type1T03[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Type1T12[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Address8To15Xor1[256] =
{
	,
//                                                              ^^^^  ^^^^  ^^^^  ^^^^
};


static const uint8_t romset_crypto_cmc42Address8To15Xor2[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Address16To23Xor1[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Address16To23Xor2[256] =
{
	,
};


static const uint8_t romset_crypto_cmc42Address0To7Xor[256] =
{
	,
};


static const uint8_t romset_crypto_cmc50Type0T03[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Type0T12[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Type1T03[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Type1T12[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Address8To15Xor1[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Address8To15Xor2[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Address16To23Xor1[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Address16To23Xor2[256] =
{
	,
};

static const uint8_t romset_crypto_cmc50Address0To7Xor[256] =
{
	,
};

static const uint8_t romset_crypto_m1Address8To15Xor[256] =
{
		,
};

static const uint8_t romset_crypto_m1Address0To7Xor[256] =
{
		,
};

static uint16_t
romset_crypto_bitswap16(uint16_t value, const int *bits)
{
    uint16_t result = 0;
    for (int index = 0; index < 16; ++index) {
        result = (uint16_t)((result << 1) | ((value >> bits[index]) & 1u));
    }
    return result;
}

static uint32_t
romset_crypto_bitswap32(uint32_t value, const int *bits, int count)
{
    uint32_t result = 0;
    for (int index = 0; index < count; ++index) {
        result = (result << 1) | ((value >> bits[index]) & 1u);
    }
    return result;
}

static uint16_t
romset_crypto_read16le(const uint8_t *data, size_t offset)
{
    return (uint16_t)(data[offset] | ((uint16_t)data[offset + 1u] << 8));
}

static uint32_t
romset_crypto_read32Native(const uint8_t *data)
{
    uint32_t value;
    memcpy(&value, data, sizeof(value));
    return value;
}

static void
romset_crypto_write16le(uint8_t *data, size_t offset, uint16_t value)
{
    data[offset] = (uint8_t)(value & 0xffu);
    data[offset + 1u] = (uint8_t)(value >> 8);
}

static void
romset_crypto_write32Native(uint8_t *data, uint32_t value)
{
    memcpy(data, &value, sizeof(value));
}

static void
romset_crypto_cmcDecryptPair(uint8_t *r0,
                             uint8_t *r1,
                             uint8_t c0,
                             uint8_t c1,
                             const uint8_t *table0hi,
                             const uint8_t *table0lo,
                             const uint8_t *table1,
                             const uint8_t *address0_7Xor,
                             uint32_t base,
                             int invert)
{
    uint8_t tmp = table1[(base & 0xffu) ^ address0_7Xor[(base >> 8) & 0xffu]];
    uint8_t xor0 = (uint8_t)((table0hi[(base >> 8) & 0xffu] & 0xfeu) | (tmp & 0x01u));
    uint8_t xor1 = (uint8_t)((tmp & 0xfeu) | (table0lo[(base >> 8) & 0xffu] & 0x01u));

    if (invert) {
        *r0 = c1 ^ xor0;
        *r1 = c0 ^ xor1;
    }
    else {
        *r0 = c0 ^ xor0;
        *r1 = c1 ^ xor1;
    }
}

static int
romset_crypto_cmcGfxDecrypt(uint8_t *rom,
                            size_t romSize,
                            uint8_t extraXor,
                            const uint8_t *type0T03,
                            const uint8_t *type0T12,
                            const uint8_t *type1T03,
                            const uint8_t *type1T12,
                            const uint8_t *address8_15Xor1,
                            const uint8_t *address8_15Xor2,
                            const uint8_t *address16_23Xor1,
                            const uint8_t *address16_23Xor2,
                            const uint8_t *address0_7Xor)
{
    if (!rom || (romSize & 3u) != 0) {
        return 0;
    }

    uint8_t *buffer = (uint8_t *)malloc(romSize);
    if (!buffer) {
        return 0;
    }

    uint32_t wordCount = (uint32_t)(romSize / 4u);
    for (uint32_t rpos = 0; rpos < wordCount; ++rpos) {
        size_t offset = (size_t)rpos * 4u;
        uint32_t mid = (rpos >> 8) & 0xffu;
        romset_crypto_cmcDecryptPair(&buffer[offset + 0u],
                                     &buffer[offset + 3u],
                                     rom[offset + 0u],
                                     rom[offset + 3u],
                                     type0T03,
                                     type0T12,
                                     type1T03,
                                     address0_7Xor,
                                     rpos,
                                     (int)((rpos >> 8) & 1u));
        romset_crypto_cmcDecryptPair(&buffer[offset + 1u],
                                     &buffer[offset + 2u],
                                     rom[offset + 1u],
                                     rom[offset + 2u],
                                     type0T12,
                                     type0T03,
                                     type1T12,
                                     address0_7Xor,
                                     rpos,
                                     (int)(((rpos >> 16) ^ address16_23Xor2[mid]) & 1u));
    }

    uint8_t *dest = rom;
    for (uint32_t rpos = 0; rpos < wordCount; ++rpos) {
        uint32_t baser = rpos;
        baser ^= extraXor;
        uint32_t base0 = baser & 0xffu;
        uint32_t base1 = (baser >> 8) & 0xffu;
        uint32_t base2 = (baser >> 16) & 0xffu;

        base1 ^= address8_15Xor1[base2] ^ address8_15Xor2[base0];
        base2 ^= address16_23Xor1[base0] ^ address16_23Xor2[base1];
        base0 ^= address0_7Xor[base1];
        baser = base0 | (base1 << 8) | (base2 << 16);
        baser &= wordCount - 1u;

        romset_crypto_write32Native(dest, romset_crypto_read32Native(buffer + (size_t)baser * 4u));
        dest += 4u;
    }

    free(buffer);
    return 1;
}

int
romset_crypto_cmc42GfxDecrypt(uint8_t *rom, size_t romSize, uint8_t extraXor)
{
    return romset_crypto_cmcGfxDecrypt(rom,
                                       romSize,
                                       extraXor,
                                       romset_crypto_cmc42Type0T03,
                                       romset_crypto_cmc42Type0T12,
                                       romset_crypto_cmc42Type1T03,
                                       romset_crypto_cmc42Type1T12,
                                       romset_crypto_cmc42Address8To15Xor1,
                                       romset_crypto_cmc42Address8To15Xor2,
                                       romset_crypto_cmc42Address16To23Xor1,
                                       romset_crypto_cmc42Address16To23Xor2,
                                       romset_crypto_cmc42Address0To7Xor);
}

int
romset_crypto_cmc50GfxDecrypt(uint8_t *rom, size_t romSize, uint8_t extraXor)
{
    return romset_crypto_cmcGfxDecrypt(rom,
                                       romSize,
                                       extraXor,
                                       romset_crypto_cmc50Type0T03,
                                       romset_crypto_cmc50Type0T12,
                                       romset_crypto_cmc50Type1T03,
                                       romset_crypto_cmc50Type1T12,
                                       romset_crypto_cmc50Address8To15Xor1,
                                       romset_crypto_cmc50Address8To15Xor2,
                                       romset_crypto_cmc50Address16To23Xor1,
                                       romset_crypto_cmc50Address16To23Xor2,
                                       romset_crypto_cmc50Address0To7Xor);
}

void
romset_crypto_sfixDecrypt(const uint8_t *rom, size_t romSize, uint8_t *fixed, size_t fixedSize)
{
    if (!rom || !fixed || fixedSize > romSize) {
        return;
    }

    const uint8_t *src = rom + romSize - fixedSize;
    for (size_t index = 0; index < fixedSize; ++index) {
        fixed[index] = src[(index & ~(size_t)0x1f) + ((index & 0x07u) << 2) + ((~index & 0x08u) >> 2) + ((index & 0x10u) >> 4)];
    }
}

static uint16_t
romset_crypto_sum16(const uint8_t *data, size_t size)
{
    uint32_t sum = 0;
    for (size_t index = 0; index < size; ++index) {
        sum += data[index];
    }
    return (uint16_t)sum;
}

static uint32_t
romset_crypto_m1AddressScramble(uint32_t address, uint16_t key)
{
    static const int keyBits[16] = {12,0,2,4,8,15,7,13,10,1,3,6,11,9,14,5};
    static const int p1Bits[8][16] = {
        {4,5,9,6,13,11,12,0,8,3,2,1,7,10,14,15},
        {12,6,0,10,14,4,13,5,3,2,9,15,11,8,1,7},
        {9,12,11,13,5,2,0,4,1,15,7,10,3,14,6,8},
        {5,14,12,1,10,0,6,13,7,11,4,3,9,15,8,2},
        {11,0,2,5,12,7,4,9,10,8,3,14,15,6,13,1},
        {13,10,5,8,1,12,14,6,2,9,0,7,4,3,15,11},
        {2,4,7,12,0,3,9,11,14,1,15,6,8,13,5,10},
        {6,1,13,15,8,5,10,14,11,4,12,2,0,7,3,9},
    };
    static const int finalBits[16] = {7,15,14,6,5,13,12,4,11,3,10,2,9,1,8,0};

    uint32_t block = (address >> 16) & 7u;
    uint16_t aux = (uint16_t)(address & 0xffffu);
    aux ^= romset_crypto_bitswap16(key, keyBits);
    aux = romset_crypto_bitswap16(aux, p1Bits[block]);
    aux ^= romset_crypto_m1Address0To7Xor[(aux >> 8) & 0xffu];
    aux ^= (uint16_t)romset_crypto_m1Address8To15Xor[aux & 0xffu] << 8;
    aux = romset_crypto_bitswap16(aux, finalBits);

    return (block << 16) | aux;
}

int
romset_crypto_cmc50M1Decrypt(uint8_t *rom, size_t cryptSize, size_t audioSize)
{
    if (!rom || cryptSize < 0x80000u || audioSize < 0x90000u) {
        return 0;
    }

    uint8_t *buffer = (uint8_t *)malloc(0x80000u);
    if (!buffer) {
        return 0;
    }

    uint16_t key = romset_crypto_sum16(rom, 0x10000u);
    for (uint32_t index = 0; index < 0x80000u; ++index) {
        buffer[index] = rom[romset_crypto_m1AddressScramble(index, key)];
    }
    memcpy(rom, buffer, 0x10000u);
    memcpy(rom + 0x10000u, buffer, 0x80000u);
    free(buffer);
    return 1;
}

int
romset_crypto_pcm2Decrypt(uint8_t *rom, size_t romSize, int value)
{
    if (!rom || value <= 0 || (value & 1) != 0) {
        return 0;
    }

    uint8_t *buffer = (uint8_t *)malloc((size_t)value);
    if (!buffer) {
        return 0;
    }

    for (size_t byteIndex = 0; byteIndex + (size_t)value <= romSize; byteIndex += (size_t)value) {
        memcpy(buffer, rom + byteIndex, (size_t)value);
        for (int wordIndex = 0; wordIndex < value / 2; ++wordIndex) {
            int sourceWord = wordIndex ^ (value / 4);
            rom[byteIndex + (size_t)wordIndex * 2u + 0u] = buffer[(size_t)sourceWord * 2u + 0u];
            rom[byteIndex + (size_t)wordIndex * 2u + 1u] = buffer[(size_t)sourceWord * 2u + 1u];
        }
    }

    free(buffer);
    return 1;
}

static int
romset_crypto_smaNgh256Decrypt68k(uint8_t *rom, size_t romSize)
{
    if (!rom || romSize < 0x900000u) {
        return 0;
    }

    static const int dataBits[16] = {4,11,14,3,1,13,0,7,2,8,12,15,10,9,5,6};
    static const int fixedBits[19] = {18,15,2,1,13,3,0,9,6,16,4,11,5,7,12,17,14,10,8};
    static const int bankBits[15] = {2,11,0,14,6,4,13,8,9,3,10,7,5,12,1};

    for (size_t offset = 0x100000u; offset < 0x900000u; offset += 2u) {
        uint16_t value = romset_crypto_read16le(rom, offset);
        romset_crypto_write16le(rom, offset, romset_crypto_bitswap16(value, dataBits));
    }

    for (size_t wordIndex = 0; wordIndex < 0x0c0000u / 2u; ++wordIndex) {
        uint32_t sourceWord = 0x5d0000u / 2u + romset_crypto_bitswap32((uint32_t)wordIndex, fixedBits, 19);
        uint16_t value = romset_crypto_read16le(rom, (size_t)sourceWord * 2u);
        romset_crypto_write16le(rom, wordIndex * 2u, value);
    }

    uint8_t *buffer = (uint8_t *)malloc(0x10000u);
    if (!buffer) {
        return 0;
    }
    for (size_t blockOffset = 0x100000u; blockOffset < 0x900000u; blockOffset += 0x10000u) {
        memcpy(buffer, rom + blockOffset, 0x10000u);
        for (size_t wordIndex = 0; wordIndex < 0x10000u / 2u; ++wordIndex) {
            uint32_t sourceWord = romset_crypto_bitswap32((uint32_t)wordIndex, bankBits, 15);
            uint16_t value = romset_crypto_read16le(buffer, (size_t)sourceWord * 2u);
            romset_crypto_write16le(rom, blockOffset + wordIndex * 2u, value);
        }
    }
    free(buffer);

    return 1;
}

static int
romset_crypto_smaNgh253Decrypt68kWithTables(uint8_t *rom,
                                            size_t romSize,
                                            const int *dataBits,
                                            const int *fixedBits,
                                            const int *bankBits,
                                            uint32_t fixedBase)
{
    if (!rom || romSize < 0x900000u) {
        return 0;
    }

    for (size_t offset = 0x100000u; offset < 0x900000u; offset += 2u) {
        uint16_t value = romset_crypto_read16le(rom, offset);
        romset_crypto_write16le(rom, offset, romset_crypto_bitswap16(value, dataBits));
    }

    for (size_t wordIndex = 0; wordIndex < 0x0c0000u / 2u; ++wordIndex) {
        uint32_t sourceWord = fixedBase / 2u + romset_crypto_bitswap32((uint32_t)wordIndex, fixedBits, 19);
        uint16_t value = romset_crypto_read16le(rom, (size_t)sourceWord * 2u);
        romset_crypto_write16le(rom, wordIndex * 2u, value);
    }

    uint8_t *buffer = (uint8_t *)malloc(0x8000u);
    if (!buffer) {
        return 0;
    }
    for (size_t blockOffset = 0x100000u; blockOffset < 0x900000u; blockOffset += 0x8000u) {
        memcpy(buffer, rom + blockOffset, 0x8000u);
        for (size_t wordIndex = 0; wordIndex < 0x8000u / 2u; ++wordIndex) {
            uint32_t sourceWord = romset_crypto_bitswap32((uint32_t)wordIndex, bankBits, 14);
            uint16_t value = romset_crypto_read16le(buffer, (size_t)sourceWord * 2u);
            romset_crypto_write16le(rom, blockOffset + wordIndex * 2u, value);
        }
    }
    free(buffer);

    return 1;
}

static int
romset_crypto_smaNgh253Decrypt68k(uint8_t *rom, size_t romSize)
{
    static const int dataBits[16] = {13,12,14,10,8,2,3,1,5,9,11,4,15,0,6,7};
    static const int fixedBits[19] = {18,4,5,16,14,7,9,6,13,17,15,3,1,2,12,11,8,10,0};
    static const int bankBits[14] = {9,4,8,3,13,6,2,7,0,12,1,11,10,5};
    return romset_crypto_smaNgh253Decrypt68kWithTables(rom, romSize, dataBits, fixedBits, bankBits, 0x710000u);
}

static int
romset_crypto_smaNgh253RevisionBDecrypt68k(uint8_t *rom, size_t romSize)
{
    static const int dataBits[16] = {14,5,1,11,7,4,10,15,3,12,8,13,0,2,9,6};
    static const int fixedBits[19] = {18,5,16,11,2,6,7,17,3,12,8,14,4,0,9,1,10,15,13};
    static const int bankBits[14] = {12,8,1,7,11,3,13,10,6,9,5,4,0,2};
    return romset_crypto_smaNgh253Decrypt68kWithTables(rom, romSize, dataBits, fixedBits, bankBits, 0x7f8000u);
}

int
romset_crypto_applyNgh264Cmc50Pcm2(uint8_t *crom,
                                   size_t cromSize,
                                   uint8_t *srom,
                                   size_t sromSize,
                                   uint8_t *mrom,
                                   size_t mromSize,
                                   uint8_t *vrom,
                                   size_t vromSize)
{
    if (!romset_crypto_cmc50GfxDecrypt(crom, cromSize, ROMSET_NGH264_CMC50_GFX_KEY)) {
        return 0;
    }
    romset_crypto_sfixDecrypt(crom, cromSize, srom, sromSize);
    if (!romset_crypto_cmc50M1Decrypt(mrom, 0x80000u, mromSize)) {
        return 0;
    }
    if (!romset_crypto_pcm2Decrypt(vrom, vromSize, 16)) {
        return 0;
    }
    return 1;
}

int
romset_crypto_applyNgh256SmaCmc42(uint8_t *prom,
                                  size_t promSize,
                                  uint8_t *crom,
                                  size_t cromSize,
                                  uint8_t *srom,
                                  size_t sromSize)
{
    if (!romset_crypto_smaNgh256Decrypt68k(prom, promSize)) {
        return 0;
    }
    if (!romset_crypto_cmc42GfxDecrypt(crom, cromSize, ROMSET_NGH256_CMC42_GFX_KEY)) {
        return 0;
    }
    romset_crypto_sfixDecrypt(crom, cromSize, srom, sromSize);
    return 1;
}

int
romset_crypto_applyNgh253RevisionBSmaCmc42(uint8_t *prom,
                                           size_t promSize,
                                           uint8_t *crom,
                                           size_t cromSize,
                                           uint8_t *srom,
                                           size_t sromSize)
{
    if (!romset_crypto_smaNgh253RevisionBDecrypt68k(prom, promSize)) {
        return 0;
    }
    if (!romset_crypto_cmc42GfxDecrypt(crom, cromSize, ROMSET_NGH253_CMC42_GFX_KEY)) {
        return 0;
    }
    romset_crypto_sfixDecrypt(crom, cromSize, srom, sromSize);
    return 1;
}

int
romset_crypto_applyNgh253SmaCmc42(uint8_t *prom,
                                  size_t promSize,
                                  uint8_t *crom,
                                  size_t cromSize,
                                  uint8_t *srom,
                                  size_t sromSize)
{
    if (!romset_crypto_smaNgh253Decrypt68k(prom, promSize)) {
        return 0;
    }
    if (!romset_crypto_cmc42GfxDecrypt(crom, cromSize, ROMSET_NGH253_CMC42_GFX_KEY)) {
        return 0;
    }
    romset_crypto_sfixDecrypt(crom, cromSize, srom, sromSize);
    return 1;
}
