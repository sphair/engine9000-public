...
uae_u8 *restore_disk (int num,uae_u8 *src)
{
	...
	// `restore_path()` normally returns a freshly allocated string, but in some
	// restore scenarios it can alias the destination buffer. The original code:
	// _tcsncpy (changed_prefs.floppyslots[num].df, s, MAX_DPATH);
	// is undefined for overlap and triggers AddressSanitizer aborts when src/dst
	// overlap (including src == dst).
	if (s) {
		TCHAR *dstDf = changed_prefs.floppyslots[num].df;
		const uintptr_t sPtr = (uintptr_t)(const void *)s;
		const uintptr_t dPtr = (uintptr_t)(const void *)dstDf;
		const int aliasesDst = (sPtr >= dPtr && sPtr < (dPtr + (uintptr_t)MAX_DPATH)) ? 1 : 0;
		size_t len = 0;
		while (s[len] && (len + 1) < MAX_DPATH) {
			len++;
		}
		// Use memmove so we handle overlap if `s` points inside `dstDf`.
		memmove(dstDf, s, len);
		dstDf[len] = 0;
		if (!aliasesDst) {
			xfree(s);
		...
		changed_prefs.floppyslots[num].df[0] = 0;
	}
