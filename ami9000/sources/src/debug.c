...
static uae_u32 readint_3(TCHAR **c, int *size, bool *err)
{
	return readnum(c, size, '!', err);
}
...
STATIC_INLINE void putpixel_4 (uae_u8 *buf, int bpp, int x, xcolnr c8)
{
	if (x <= 0)
		return;
...
static void debug_draw_cycles(uae_u8 *buf, int bpp, int line, int width, int height, uae_u32 *xredcolors, uae_u32 *xgreencolors, uae_u32 *xbluescolors)
{
	...
	for (x = 0; x < record_dma_maxhpos; x++) {
		...
		if (dr->intlev > intlev)
			intlev = dr->intlev;
		putpixel_4(buf, bpp, xx + 4, c);
		if (xplus > 1)
			putpixel_4(buf, bpp, xx + 4 + 1, c);
		if (xplus > 2)
			putpixel_4(buf, bpp, xx + 4 + 2, c);
	}
	putpixel_4 (buf, bpp, dx + 0, 0);
	putpixel_4 (buf, bpp, dx + 1, lc(intlevc[intlev]));
	putpixel_4 (buf, bpp, dx + 2, lc(intlevc[intlev]));
	putpixel_4 (buf, bpp, dx + 3, 0);
}
...
static void REGPARAM2 debug_lput (uaecptr addr, uae_u32 v)
{
	...
	uae_u32 oldv = 0;
	int oldvalid = 0;
	if (debug_mem_banks && debug_mem_banks[off] && debug_mem_banks[off]->check(addr, 4)) {
...
static void REGPARAM2 debug_wput (uaecptr addr, uae_u32 v)
{
	...
	uae_u32 oldv = 0;
	int oldvalid = 0;
	if (debug_mem_banks && debug_mem_banks[off] && debug_mem_banks[off]->check(addr, 2)) {
...
static void REGPARAM2 debug_bput (uaecptr addr, uae_u32 v)
{
	...
	uae_u32 oldv = 0;
	int oldvalid = 0;
	if (debug_mem_banks && debug_mem_banks[off] && debug_mem_banks[off]->check(addr, 1)) {
...
int debug_enableE9kHooks(void)
{
	...
	for (int i = 0; i < membank_total; ++i) {
		...
		memcpy(newbank, orig, sizeof(addrbank));

		newbank->bget = debug_bget;
		newbank->wget = debug_wget;
		newbank->lget = debug_lget;
		newbank->bput = debug_bput;
		newbank->wput = debug_wput;
		newbank->lput = debug_lput;
		newbank->check = debug_check;
		newbank->xlateaddr = debug_xlate;
		newbank->wgeti = debug_wgeti;
		newbank->lgeti = debug_lgeti;

		if (!newbank->mask) {
			newbank->mask = (uae_u32)-1;
		}
		newbank->baseaddr_direct_r = 0;
		newbank->baseaddr_direct_w = 0;
		...
		if (more_params (c)) {
			...
			if (more_params(c)) {
				for (;;) {
					...
					if (nc == 'V') {
						mwn->val = readhex_3(c, &mwn->val_size, &err);
						mwn->val_enabled = 1;
					}
					if (!more_params(c))
						break;
...
static struct dsprintfstack debugsprintf_stack[DEBUGSPRINTF_SIZE];
