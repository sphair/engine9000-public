...
static const char *
main_sectionTypeName(ULONG type)
{
  switch (type) {
  case HUNK_CODE:
    return ".text";
...
static BOOL
main_sectionTypeToDebugType(ULONG type, ULONG *outSection)
{
  if (outSection) {
...
static void
main_formatSectionLabel(ULONG type, ULONG index, char *out, ULONG outCap)
{
  const char *sectionName = main_sectionTypeName(type);
  ULONG writePos = 0;
...

  {
    ...
    do {
      digits[digitCount++] = (char)('0' + (value % 10UL));
      value /= 10UL;
    } while (value != 0UL && digitCount < (ULONG)sizeof(digits));

    while (digitCount > 0UL && (writePos + 1UL) < outCap) {
      out[writePos++] = digits[--digitCount];
    }
    out[writePos] = 0;
  }
}
...
PrintSegList(BPTR seglist, const SegType *types, ULONG typeCount, int breakEnabled)
{
  ...
  while (seg) {
    ...
    if (haveType) {
      main_formatSectionLabel(t, idx, sectionLabel, sizeof(sectionLabel));

      if (breakEnabled && !haveBreak && t == HUNK_CODE) {
        ULONG breakAddr = (ULONG)base;
        ULONG breakAddr2 = ((ULONG)base) + 2;
        printf("engine9000: setting entry breakpoint=%08x from %s\n",
               (ULONG)breakAddr,
      ...
      if (main_sectionTypeToDebugType(t, &pushSection)) {
        push = 1;
      }
      ...
      if (idx == 0) {
        ...
        printf("engine9000: fallback push .text base=%08x\n", (ULONG)base);
        push = 1;
        pushSection = ENGINE_9000_DEBUG_SECTION_TEXT;
      } else if (idx == 1) {
        strcpy(sectionLabel, ".data");
        printf("engine9000: fallback push .data base=%08x\n", (ULONG)base);
        push = 1;
        pushSection = ENGINE_9000_DEBUG_SECTION_DATA;
      } else if (idx == 2) {
        strcpy(sectionLabel, ".bss");
        printf("engine9000: fallback push .bss base=%08x\n", (ULONG)base);
        push = 1;
    ...
    if (push) {
      printf("engine9000: push %s base=%08x size=%08x\n",
